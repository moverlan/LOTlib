...
class Node(object):
    ...
    def copy(self):
        if not self.is_root:
            raise Exception, 'can only copy full trees'
        return self._copy()
    ...
    def _copy(self):
        ...
        if new.children:
            ...
            #print 'self is', id(self), 'new is', id(new)
            for i, child in enumerate(self.children):
                new._children[i] = child._copy()
                new._children[i]._parent = new
                #print ' ', id(self), 'child', i, 'is', id(self.child(i))
                #print ' ', id(self.child(i)), 'parent is', id(self.child(i).parent)
                #print ' ', id(new), 'child', i, 'is', id(new.child(i))
                #print ' ', id(new.child(i)), 'parent is', id(new.child(i).parent)
                assert child is not new._children[i]
